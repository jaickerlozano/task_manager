...
interface TaskModalUpdateProps {
  isOpen: boolean;
  onClose: () => void;
  onUpdateTask: (taskId: number, taskTitle: string, taskDescription: string) => void;
  taskId: number;
  initialTitle: string;
...
export const TaskModalUpdate: React.FC<TaskModalUpdateProps> = ({ isOpen, onClose, onUpdateTask, taskId, initialTitle, initialDescription }) => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    onUpdateTask(taskId, taskTitle, taskDescription);
    onClose();
  };
